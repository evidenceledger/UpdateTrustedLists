...
# p = subprocess.run(["git"], capture_output=True)
...
while True:
    t = datetime.today()
    if t.hour == 23 and t.minute == 8:
        if must_execute:
            print("Updating Trusted Lists")
            download_and_update_all()
            subprocess.run(["git", "add", "."])
            subprocess.run(["git", "commit", "-m", "Update Trusted Lists"])
            subprocess.run(["git", "push", "-u", "origin", "main"])

        must_execute = False
        print("Must execute = False")

    else:
        must_execute = True
        print("Must execute = True")
    time.sleep(10)
